...
|---|---|---|
| **"orchestrated via LangGraph"**, "implemented using LangGraph, with each reasoning step structured as a node in a directed graph" (§3.1, §3.4, §3.7, Week 4) | **No LangGraph.** `langchain`/`langgraph` are listed in `pyproject.toml`/`requirements.txt` but **never imported** anywhere in `src/` or `app/` (verified by grep). The loop is a hand-written `_run_react_loop` over Groq's native function-calling (`agent/loop.py`). | **High** — central architecture claim is false. |
